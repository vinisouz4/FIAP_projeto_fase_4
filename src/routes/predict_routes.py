...
from src.services.predict_service import _build_features, _build_input
from src.services.model_loader_services import get_model, get_metadata
from src.services.auth_services import current_user
from src.monitoring.metrics_middleware import record_prediction
from src.monitoring.model_monitor import log_prediction, PredictionRecord
...
async def predict(body: PredictRequest, user: str = user_dependency):
    ...
    record_prediction( # → Prometheus (síncrono, instantâneo)
        direction=direction,
    ...
    log_prediction(PredictionRecord( # → MLflow (buffered, sem impacto na latência)
        ticker=body.ticker.upper(),
        predicted_return=predicted_pct,
        direction=direction,
        confidence=confidence,
